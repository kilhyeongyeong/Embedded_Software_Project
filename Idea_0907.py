    ...
    while True: 
        ...
        if check_EWSN == True:
            if find_alphabet == False:
                ...
                left_pixel = len(np.where(img_binary[:, :160] == 0)[0])
                right_pixel = len(np.where(img_binary[:, 160:320] == 0)[0])
