'******** 2족 보행로봇 초기 영점 프로그램 ********

DIM I AS BYTE
DIM J AS BYTE
DIM MODE AS BYTE
DIM A AS BYTE
DIM A_old AS BYTE
DIM B AS BYTE
DIM C AS BYTE
DIM 보행속도 AS BYTE
DIM 좌우속도 AS BYTE
DIM 좌우속도2 AS BYTE
DIM 보행순서 AS BYTE
DIM 현재전압 AS BYTE
DIM 반전체크 AS BYTE
DIM 모터ONOFF AS BYTE
DIM 자이로ONOFF AS BYTE
DIM 기울기앞뒤 AS INTEGER
DIM 기울기좌우 AS INTEGER
DIM DELAY_TIME AS BYTE
DIM DELAY_TIME2 AS BYTE
'DIM STEP AS BYTE
DIM 넘어진확인 AS BYTE
DIM 기울기확인횟수 AS BYTE
DIM 보행횟수 AS BYTE
DIM 보행COUNT AS BYTE
'************************************************
DIM NO_0 AS BYTE
DIM NO_1 AS BYTE
DIM NO_2 AS BYTE
DIM NO_3 AS BYTE
DIM NO_4 AS BYTE

DIM NUM AS BYTE

DIM BUTTON_NO AS INTEGER
DIM SOUND_BUSY AS BYTE
DIM TEMP_INTEGER AS INTEGER

'**** 기울기센서포트 설정 ****
CONST 앞뒤기울기AD포트 = 0
CONST 좌우기울기AD포트 = 1
CONST 기울기확인시간 = 20  'ms


CONST min = 61	'뒤로넘어졌을때
CONST max = 107	'앞으로넘어졌을때
CONST COUNT_MAX = 3
CONST 하한전압 = 154  '약6V전압
CONST 머리이동속도 = 10
'************************************************



PTP SETON 				'단위그룹별 점대점동작 설정
PTP ALLON				'전체모터 점대점 동작 설정

DIR G6A,1,0,0,1,0,0		'모터0~5번
DIR G6D,0,1,1,0,1,1		'모터18~23번
DIR G6B,1,1,1,1,1,1		'모터6~11번
DIR G6C,0,0,0,0,0,0		'모터12~17번    '0,0,0,0,1,0

'************************************************

OUT 52,0	'머리 LED 켜기
'***** 초기선언 '************************************************
'STEP = 0
보행순서 = 0
반전체크 = 0
기울기확인횟수 = 0
보행횟수 = 1
모터ONOFF = 0

'****초기위치 피드백*****************************

GOSUB 자이로INIT
GOSUB MOTOR_SET
TEMPO 230
MUSIC "g<abcdefg"
DELAY 1000

SPEED 5
GOSUB MOTOR_ON

'delay 3000

GOSUB 전원초기자세
GOSUB 기본자세
GOSUB 자이로MID2
GOSUB 자이로ON
GOSUB All_motor_mode3


'PRINT "VOLUME 200 !"
'PRINT "SOUND 12 !" '안녕하세요

GOTO MAIN	'시리얼 수신 루틴으로 가기

'************************************************


'************************************************
시작음:
    TEMPO 220
    MUSIC "O23EAB7EA>3#C"
    RETURN
    '************************************************
종료음:
    TEMPO 220
    MUSIC "O38GD<BGD<BG"
    RETURN
    '************************************************
에러음:
    TEMPO 250
    MUSIC "FFF"
    RETURN
    '************************************************
    '************************************************
MOTOR_ON: '전포트서보모터사용설정

    GOSUB MOTOR_GET

    MOTOR G6B
    DELAY 50
    MOTOR G6C
    DELAY 50
    MOTOR G6A
    DELAY 50
    MOTOR G6D

    모터ONOFF = 0
    'GOSUB 시작음			
    RETURN

    '************************************************
    '전포트서보모터사용설정
MOTOR_OFF:

    MOTOROFF G6B
    MOTOROFF G6C
    MOTOROFF G6A
    MOTOROFF G6D
    모터ONOFF = 1	
    GOSUB MOTOR_GET	
    'GOSUB 종료음	
    RETURN
    '************************************************
    '위치값피드백
MOTOR_GET:
    GETMOTORSET G6A,1,1,1,1,1,0
    GETMOTORSET G6B,1,1,1,0,0,1
    GETMOTORSET G6C,1,1,1,0,0,0
    GETMOTORSET G6D,1,1,1,1,1,0
    RETURN

    '************************************************
    '위치값피드백
MOTOR_SET:
    GETMOTORSET G6A,1,1,1,1,1,0
    GETMOTORSET G6B,1,1,1,0,0,1
    GETMOTORSET G6C,1,1,1,0,1,0
    GETMOTORSET G6D,1,1,1,1,1,0
    RETURN

    '************************************************
All_motor_Reset:

    MOTORMODE G6A,1,1,1,1,1,1
    MOTORMODE G6D,1,1,1,1,1,1
    MOTORMODE G6B,1,1,1,,,1
    MOTORMODE G6C,1,1,1,,1

    RETURN
    '************************************************
All_motor_mode2:

    MOTORMODE G6A,2,2,2,2,2
    MOTORMODE G6D,2,2,2,2,2
    MOTORMODE G6B,2,2,2,,,2
    MOTORMODE G6C,2,2,2,,2

    RETURN
    '************************************************
All_motor_mode3:

    MOTORMODE G6A,3,3,3,3,3
    MOTORMODE G6D,3,3,3,3,3
    MOTORMODE G6B,3,3,3,,,3
    MOTORMODE G6C,3,3,3,,3

    RETURN
    '************************************************
Leg_motor_mode1:
    MOTORMODE G6A,1,1,1,1,1
    MOTORMODE G6D,1,1,1,1,1
    RETURN
    '************************************************
Leg_motor_mode2:
    MOTORMODE G6A,2,2,2,2,2
    MOTORMODE G6D,2,2,2,2,2
    RETURN

    '************************************************
Leg_motor_mode3:
    MOTORMODE G6A,3,3,3,3,3
    MOTORMODE G6D,3,3,3,3,3
    RETURN
    '************************************************
Leg_motor_mode4:
    MOTORMODE G6A,3,2,2,1,3
    MOTORMODE G6D,3,2,2,1,3
    RETURN
    '************************************************
Leg_motor_mode5:
    MOTORMODE G6A,3,2,2,1,2
    MOTORMODE G6D,3,2,2,1,2
    RETURN
    '************************************************
Leg_motor_mode6:
    MOTORMODE G6A,3,3,3,3,2
    MOTORMODE G6D,3,3,3,3,2
    RETURN
    '*************************************************
LArm_motor_mode1:
    MOTORMODE G6B,1,1,1,,,1
    RETURN
    '************************************************
Arm_motor_mode1:
    MOTORMODE G6B,1,1,1,,,1
    MOTORMODE G6C,1,1,1,,1
    RETURN
    '************************************************
Arm_motor_mode2:
    MOTORMODE G6B,2,2,2,,,2
    MOTORMODE G6C,2,2,2,,2
    RETURN

    '************************************************
Arm_motor_mode3:
    MOTORMODE G6B,3,3,3,,,3
    MOTORMODE G6C,3,3,3,,3
    RETURN
    '************************************************
    '***********************************************
    '***********************************************
    '**** 자이로감도 설정 ****
자이로INIT:

    GYRODIR G6A, 0, 0, 1, 0,0
    GYRODIR G6D, 1, 0, 1, 0,0
    GYRODIR G6B, 1, 0, 0, 0,0
    GYRODIR G6C, 1, 0, 0, 0,0

    GYROSENSE G6A,200,150,30,150,0
    GYROSENSE G6D,200,150,30,150,0



    RETURN
    '***********************************************
    '**** 자이로감도 설정 ****
자이로MAX:

    GYROSENSE G6A,250,180,30,180,0
    GYROSENSE G6D,250,180,30,180,0

    RETURN
    '***********************************************
자이로MID:

    GYROSENSE G6A,200,150,30,150,0
    GYROSENSE G6D,200,150,30,150,0

    RETURN
    '***********************************************
자이로MID2:

    GYROSENSE G6A,250,100,30,100,
    GYROSENSE G6D,250,100,30,100,

    RETURN
    '***********************************************
자이로MIN:

    GYROSENSE G6A,200,100,30,100,0
    GYROSENSE G6D,200,100,30,100,0
    RETURN
    '***********************************************
자이로ON:


    GYROSET G6A, 4, 3, 3, 3, 0
    GYROSET G6D, 4, 3, 3, 3, 0


    자이로ONOFF = 1

    RETURN
    '***********************************************
자이로OFF:

    GYROSET G6A, 0, 0, 0, 0, 0
    GYROSET G6D, 0, 0, 0, 0, 0

    자이로ONOFF = 0
    RETURN

    '************************************************
전원초기자세:

    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  90
    MOVE G6C,100,  35,  90
    WAIT
    mode = 0
    RETURN

    '************************************************
안정화자세:
    MOVE G6A, 98,  76, 145,  93, 101, 100
    MOVE G6D, 98,  76, 145,  93, 101, 100
    MOVE G6B,100,  35,  90,
    MOVE G6C,100,  35,  90
    WAIT
    mode = 0
    RETURN
    '************************************************
기본자세:
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 25
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 117, 100
    WAIT
    mode = 0
    ETX 4800,33
    RETURN
    '************************************************
기본자세2:
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80,
    MOVE G6C,100,  35,  80, 100, 180
    WAIT
    mode = 0
    RETURN
    '************************************************
ST: '문열때 화살표확인
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, , ,110
    MOVE G6C,100,  35,  80, 100, 110
    WAIT
    mode = 0
    RETURN
    '************************************************
차렷자세:
    MOVE G6A,100, 56, 182, 76, 100, 100
    MOVE G6D,100, 56, 182, 76, 100, 100
    MOVE G6B,100, 30, 90, 100, 100, 100
    MOVE G6C,100, 30, 90, 100, 100, 100
    WAIT
    mode = 2
    RETURN
    '******************************************
앉은자세:
    MOVE G6A,100, 144,  22, 145, 100, 100
    MOVE G6D,100, 146,  22, 145, 100, 100
    MOVE G6B,110,  35,  80, 100, 100, 100
    MOVE G6C,110,  35,  80, 100, 100, 100	
    WAIT
    mode = 1
    GOSUB 자이로OFF
    RETURN
    '**********************************************
기본125:
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 25
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 125, 100
    WAIT
    mode = 0
    RETURN
    '**********************************************
기본130:
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 25
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 130, 100
    WAIT
    mode = 0
    RETURN
    '**********************************************
RX_EXIT:

    ERX 4800, A, MAIN

    GOTO RX_EXIT

GOSUB_RX_EXIT:

    ERX 4800, A, GOSUB_RX_EXIT2

    GOTO GOSUB_RX_EXIT

GOSUB_RX_EXIT2:
    RETURN
    '**********************************************
EAST:
    PRINT "VOLUME 200 !"
    PRINT "SOUND 0 !"

    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,180,  35,  80, 100, 110, 100
    WAIT

    PRINT "VOLUME 200 !"
    PRINT "SOUND 0 !"

    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 117, 100
    WAIT

    mode = 0
    ETX 4800,33
    RETURN
    '**********************************************
WEST:
    PRINT "VOLUME 200 !"
    PRINT "SOUND 1 !"

    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,180,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 117, 100
    WAIT

    PRINT "VOLUME 200 !"
    PRINT "SOUND 1 !"

    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 117, 100
    WAIT

    mode = 0
    ETX 4800,33
    RETURN
    '**********************************************
SOUTH:
    PRINT "VOLUME 200 !"
    PRINT "SOUND 2 !"

    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,20,  35,  80, 100, 100, 100
    MOVE G6C,20,  35,  80, 100, 117, 100
    WAIT

    PRINT "VOLUME 200 !"
    PRINT "SOUND 2 !"

    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 117, 100
    WAIT

    mode = 0
    ETX 4800,33
    RETURN
    '**********************************************
NORTH:
    PRINT "VOLUME 200 !"
    PRINT "SOUND 3 !"

    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,180,  35,  80, 100, 100, 100
    MOVE G6C,180,  35,  80, 100, 117, 100
    WAIT

    PRINT "VOLUME 200 !"
    PRINT "SOUND 3 !"

    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 117, 100
    WAIT

    mode = 0
    ETX 4800,33
    RETURN
    '**********************************************
safe:
    PRINT "VOLUME 200 !"
    PRINT "SOUND 4 !"

    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    WAIT
    RETURN
    '*************************************************
danger:
    PRINT "VOLUME 200 !"
    PRINT "SOUND 5 !"

    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    WAIT
    RETURN
    '************************************************
Aland:
    PRINT "VOLUME 200 !"
    PRINT "SOUND 6 !"

    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    WAIT
    RETURN
    '************************************************
Bland:
    PRINT "VOLUME 200 !"
    PRINT "SOUND 7 !"

    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    WAIT
    RETURN
    '************************************************
Cland:
    PRINT "VOLUME 200 !"
    PRINT "SOUND 8 !"

    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    WAIT
    RETURN
    '************************************************
Dland:
    PRINT "VOLUME 200 !"
    PRINT "SOUND 9 !"

    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    WAIT
    RETURN
    '************************************************
첫부분고개살짝들기:
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80,
    MOVE G6C,100,  35,  80, 100, 100
    WAIT
    mode = 0
    RETURN
    '**********************************************
WooWalk2:

    GOSUB 자이로ON
    HIGHSPEED SETOFF
    GOSUB All_motor_mode3

    SPEED 10
    MOVE G6A,100,  76, 145,  95, 100, 100
    MOVE G6D,100,  76, 145,  95, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 117, 100
    WAIT

    SPEED 10
    MOVE G6D,108,  76, 145,  95, 104, 100
    MOVE G6A,100,  86, 125, 109, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 117, 100
    WAIT

    '****************************************

    FOR I = 0 TO 2

        SPEED 10
        MOVE G6A, 99,  66, 145, 103, 100, 100
        MOVE G6D,100,  86, 145,  83, 100, 100
        MOVE G6B,100,  35,  80, 100, 100, 100
        MOVE G6C,100,  35,  80, 100, 117, 100
        WAIT

        SPEED 10
        MOVE G6A,108,  76, 145,  93, 104, 100
        MOVE G6D,100,  86, 125, 103, 100, 100
        MOVE G6B,100,  35,  80, 100, 100, 100
        MOVE G6C,100,  35,  80, 100, 117, 100
        WAIT

        SPEED 10
        MOVE G6D,101,  66, 145, 103, 100, 100
        MOVE G6A,100,  86, 145,  83, 100, 100
        MOVE G6B,100,  35,  80, 100, 100, 100
        MOVE G6C,100,  35,  80, 100, 117, 100
        WAIT

        SPEED 10
        MOVE G6D,108,  76, 145,  93, 104, 100
        MOVE G6A,100,  86, 125, 103, 100, 100
        MOVE G6B,100,  35,  80, 100, 100, 100
        MOVE G6C,100,  35,  80, 100, 117, 100
        WAIT

    NEXT I

    '****************************************

    SPEED 10
    MOVE G6A, 99,  66, 145, 103, 100, 100
    MOVE G6D,100,  86, 145,  83, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 117, 100
    WAIT

    SPEED 10
    MOVE G6A,108,  76, 145,  93, 104, 100
    MOVE G6D,100,  86, 125, 103, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 117, 100
    WAIT

    SPEED 10
    MOVE G6D,101,  66, 145, 103, 100, 100
    MOVE G6A,100,  86, 145,  83, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 117, 100
    WAIT

    SPEED 10
    MOVE G6D,108,  76, 145,  91, 104, 100
    MOVE G6A,100,  86, 125,  97, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 117, 100
    WAIT

    '****************************************

    SPEED 10
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 117, 100
    WAIT

    GOSUB 기본자세

    GOSUB 자이로OFF

    DELAY 100

    RETURN
    '***************************
CDWalk:
    GOSUB 자이로ON
    HIGHSPEED SETOFF
    GOSUB All_motor_mode3

    SPEED 15
    MOVE G6A,100,  76, 145,  97, 100, 100
    MOVE G6D,100,  76, 145,  97, 100, 100
    MOVE G6B,100,  35,  80, 100
    MOVE G6C,100,  35,  80, 100
    WAIT

    SPEED 15
    MOVE G6D,103,  76, 145,  100, 102, 100
    MOVE G6A,98,  86, 125, 111, 100, 100
    MOVE G6B,100,  35,  80, 100
    MOVE G6C,100,  35,  80, 100
    WAIT

    '****************************************

    FOR I = 0 TO 3

        SPEED 15'왼
        MOVE G6A, 100,  66, 147, 110, 100, 100
        MOVE G6D,100,  86, 145,  88, 101 100
        MOVE G6B,100,  35,  80, 100
        MOVE G6C,100,  35,  80, 100
        WAIT

        SPEED 15'오
        MOVE G6A,103,  76, 145,  97, 103, 100
        MOVE G6D,98,  86, 125, 110, 100, 100
        MOVE G6B,100,  35,  80, 100
        MOVE G6C,100,  35,  80, 100
        WAIT

        SPEED 15'왼
        MOVE G6D,100,  66, 147, 110, 100, 100
        MOVE G6A,100,  86, 145,  88, 101, 100
        MOVE G6B,100,  35,  80, 100
        MOVE G6C,100,  35,  80, 100
        WAIT

        SPEED 15'오
        MOVE G6D,103,  76, 145,  97, 103, 100
        MOVE G6A,98,  86, 125, 110, 100, 100
        MOVE G6B,100,  35,  80, 100
        MOVE G6C,100,  35,  80, 100
        WAIT

    NEXT I

    '****************************************

    SPEED 15
    MOVE G6A, 100,  66, 147, 110, 100, 100
    MOVE G6D,100,  86, 145,  88, 101, 100
    MOVE G6B,100,  35,  80, 100
    MOVE G6C,100,  35,  80, 100
    WAIT

    SPEED 15
    MOVE G6A,103,  76, 145,  97, 103, 100
    MOVE G6D,98,  86, 125, 110, 100, 100
    MOVE G6B,100,  35,  80, 100
    MOVE G6C,100,  35,  80, 100
    WAIT

    SPEED 15
    MOVE G6D,100,  66, 147, 110, 100, 100
    MOVE G6A,100,  86, 145,  88, 101, 100
    MOVE G6B,100,  35,  80, 100
    MOVE G6C,100,  35,  80, 100
    WAIT

    SPEED 15
    MOVE G6D,103,  76, 145,  97, 103, 100
    MOVE G6A,98,  80, 128,  110, 100, 100
    MOVE G6B,100,  35,  80, 100
    MOVE G6C,100,  35,  80, 100
    WAIT

    '****************************************

    SPEED 10
    MOVE G6A,100,  76, 145,  97, 100, 100
    MOVE G6D,100,  76, 145,  97, 100, 100
    MOVE G6B,100,  35,  80, 100
    MOVE G6C,100,  35,  80, 100
    WAIT

    GOSUB 기본자세

    GOSUB 자이로OFF

    DELAY 100

    RETURN
    '**********************************************
ADWalk:
    GOSUB 자이로ON
    HIGHSPEED SETOFF
    GOSUB All_motor_mode3

    SPEED 15
    MOVE G6A,100,  76, 145,  97, 100, 100
    MOVE G6D,100,  76, 145,  97, 100, 100
    MOVE G6B,100,  35,  80, 100
    MOVE G6C,100,  35,  80, 100
    WAIT

    SPEED 15
    MOVE G6D,103,  76, 145,  100, 102, 100
    MOVE G6A,98,  86, 125, 111, 100, 100
    MOVE G6B,100,  35,  80, 100
    MOVE G6C,100,  35,  80, 100
    WAIT

    '****************************************

    FOR I = 0 TO 3

        SPEED 15'왼
        MOVE G6A, 100,  66, 147, 110, 100, 100
        MOVE G6D,100,  86, 145,  88, 101 100
        MOVE G6B,100,  35,  80, 100
        MOVE G6C,100,  35,  80, 100
        WAIT

        SPEED 15'오
        MOVE G6A,102,  76, 145,  97, 103, 100
        MOVE G6D,99,  84, 127, 110, 100, 100
        MOVE G6B,100,  35,  80, 100
        MOVE G6C,100,  35,  80, 100
        WAIT

        SPEED 15'왼
        MOVE G6D,100,  66, 147, 110, 100, 100
        MOVE G6A,100,  86, 145,  88, 101, 100
        MOVE G6B,100,  35,  80, 100
        MOVE G6C,100,  35,  80, 100
        WAIT

        SPEED 15'오
        MOVE G6D,102,  76, 145,  97, 103, 100
        MOVE G6A,99,  84, 127, 110, 100, 100
        MOVE G6B,100,  35,  80, 100
        MOVE G6C,100,  35,  80, 100
        WAIT

    NEXT I

    '****************************************

    SPEED 15
    MOVE G6A, 100,  66, 147, 110, 100, 100
    MOVE G6D,100,  86, 145,  88, 101, 100
    MOVE G6B,100,  35,  80, 100
    MOVE G6C,100,  35,  80, 100
    WAIT

    SPEED 15
    MOVE G6A,102,  76, 145,  97, 103, 100
    MOVE G6D,99,  84, 127, 110, 100, 100
    MOVE G6B,100,  35,  80, 100
    MOVE G6C,100,  35,  80, 100
    WAIT

    SPEED 15
    MOVE G6D,100,  66, 147, 110, 100, 100
    MOVE G6A,100,  86, 145,  88, 101, 100
    MOVE G6B,100,  35,  80, 100
    MOVE G6C,100,  35,  80, 100
    WAIT

    SPEED 15
    MOVE G6D,102,  76, 145,  97, 103, 100
    MOVE G6A,99,  80, 129,  110, 100, 100
    MOVE G6B,100,  35,  80, 100
    MOVE G6C,100,  35,  80, 100
    WAIT

    '****************************************

    SPEED 10
    MOVE G6A,100,  76, 145,  97, 100, 100
    MOVE G6D,100,  76, 145,  97, 100, 100
    MOVE G6B,100,  35,  80, 100
    MOVE G6C,100,  35,  80, 100
    WAIT

    GOSUB 기본자세

    GOSUB 자이로OFF

    DELAY 100

    RETURN
    '**********************************************
Lturn90:

    GOSUB 자이로OFF

    GOSUB Leg_motor_mode1

    SPEED 15
    MOVE G6A,95, 104, 145,  65, 105, 100
    MOVE G6D,95,  48, 145,  121, 105, 100
    WAIT

    SPEED 15
    MOVE G6A,90, 104, 145,  65, 105, 100
    MOVE G6D,90,  48, 145,  121, 105, 100
    WAIT

    SPEED 10
    GOSUB 기본자세
    DELAY 100

    GOSUB 자이로ON
    RETURN
    '**********************************************
MLturn90:

    GOSUB 자이로OFF

    GOSUB Leg_motor_mode1

    SPEED 15
    MOVE G6A,95, 104, 145,  65, 105, 100
    MOVE G6D,95,  48, 145,  121, 105, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT

    SPEED 15
    MOVE G6A,90, 104, 145,  65, 105, 100
    MOVE G6D,90,  48, 145,  121, 105, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT

    SPEED 10
    MOVE G6A,100, 76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT

    DELAY 100

    GOSUB 자이로ON
    RETURN
    '**********************************************
jiwon:

    GOSUB 자이로OFF
    GOSUB leg_motor_mode1

    SPEED 5
    MOVE G6A,100, 150,  28, 140, 100, 100
    MOVE G6D,100, 150,  28, 140, 100, 100
    MOVE G6B,110,  35,  80, 100, 100, 100
    MOVE G6C,110,  35,  80, 100, 100, 100	
    WAIT
    DELAY 100

    SPEED 5
    MOVE G6A,100, 150,  28, 140, 100, 100
    MOVE G6D,100, 150,  28, 140, 100, 100
    MOVE G6B,160,  35,  80, 100, 100, 100
    MOVE G6C,160,  35,  80, 100, 100, 100	
    WAIT

    SPEED 5
    MOVE G6A, 60, 150,  28, 155, 140, 100
    MOVE G6D, 60, 150,  28, 155, 140, 100
    MOVE G6B,170,  35,  80, 100, 100, 100
    MOVE G6C,170,  35,  80, 100, 100, 100	
    WAIT

    SPEED 5
    MOVE G6A, 60, 162,  30, 162, 145, 100
    MOVE G6D, 60, 162,  30, 162, 145, 100
    MOVE G6B,170,  35, 80, 100, 100, 100
    MOVE G6C,170,  35, 80, 100, 100, 100
    WAIT

    SPEED 1
    MOVE G6A, 65, 157,  40, 162, 155, 100
    MOVE G6D, 65, 157,  40, 162, 155, 100
    MOVE G6B,170,  15, 55, 100, 100, 100
    MOVE G6C,170,  15, 55, 100, 100, 100
    WAIT

    '****************************************************************
    SPEED 5
    MOVE G6A, 60, 162,  30, 162, 145, 100
    MOVE G6D, 60, 162,  30, 162, 145, 100
    MOVE G6B,190,  15, 55, 100, 100, 100
    MOVE G6C,190,  15, 55, 100, 100, 100
    WAIT

    SPEED 5
    MOVE G6A, 60, 150,  28, 155, 140, 100
    MOVE G6D, 60, 150,  28, 155, 140, 100
    MOVE G6B,190,  15,  55, 100, 100, 100
    MOVE G6C,190,  15,  55, 100, 100, 100
    WAIT

    SPEED 5
    MOVE G6A,100, 150,  28, 140, 100, 100
    MOVE G6D,100, 150,  28, 140, 100, 100
    MOVE G6B,190,  15,  55, 100, 100, 100
    MOVE G6C,190,  15,  55, 100, 100, 100
    WAIT
    DELAY 100


    SPEED 7
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT

    SPEED 7
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT



    DELAY 100
    RETURN
    '***************************************
milkwalkfront:

    GOSUB 자이로ON
    HIGHSPEED SETOFF
    GOSUB All_motor_mode3

    SPEED 10
    MOVE G6A,100,  76, 143,  90, 100, 100
    MOVE G6D,100,  76, 146,  90, 100, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT

    SPEED 10
    MOVE G6D,103,  76, 144,  93, 102, 100
    MOVE G6A,98,  86, 126, 105, 100, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT

    '****************************************

   FOR I = 0 TO 1
   
    SPEED 15'왼
    MOVE G6A, 100,  70, 148, 100, 100, 100
    MOVE G6D,100,  86, 145,  81, 101 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT

    SPEED 15'오
    MOVE G6A,103,  76, 146,  92, 103, 100
    MOVE G6D,98,  86, 132, 104, 100, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT

    SPEED 15'왼
    MOVE G6D,100,  70, 148, 100, 100, 100
    MOVE G6A,100,  86, 145,  81, 101, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT

    SPEED 15'오
    MOVE G6D,103,  76, 146,  92, 103, 100
    MOVE G6A,98,  86, 132, 104, 100, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT
    
    NEXT I

    '****************************************
    SPEED 15
    MOVE G6A, 100,  70, 146, 100, 100, 100
    MOVE G6D,100,  86, 145,  81, 101, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT

    SPEED 15
    MOVE G6A,103,  76, 143,  92, 103, 100
    MOVE G6D,98,  86, 132, 101, 100, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT

    SPEED 15
    MOVE G6D,100,  70, 146, 100, 100, 100
    MOVE G6A,100,  86, 145,  81, 101, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT

    SPEED 15
    MOVE G6D,103,  76, 143,  92, 103, 100
    MOVE G6A,98,  80, 130,  101, 100, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT

    SPEED 10
    MOVE G6A,100,  76, 143,  90, 100, 100
    MOVE G6D,100,  76, 145,  90, 100, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT


    DELAY 100

    RETURN

    '****************************************
jiwondown:

    GOSUB 자이로OFF
    GOSUB leg_motor_mode1

    SPEED 5
    MOVE G6A,100, 150,  28, 140, 100, 100
    MOVE G6D,100, 150,  28, 140, 100, 100
    MOVE G6B,165, 15,  55, 100, 160, 100
    MOVE G6C,165,  15,  55, 100, 120, 100
    WAIT
    DELAY 100

    SPEED 5
    MOVE G6A,100, 150,  28, 140, 100, 100
    MOVE G6D,100, 150,  28, 140, 100, 100
    MOVE G6B,190, 15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 120, 100
    WAIT

    SPEED 5
    MOVE G6A, 60, 150,  28, 155, 140, 100
    MOVE G6D, 60, 150,  28, 155, 140, 100
    MOVE G6B,190, 15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 120, 100
    WAIT

    SPEED 5
    MOVE G6A, 60, 162,  30, 162, 145, 100
    MOVE G6D, 60, 162,  30, 162, 145, 100
    MOVE G6B,190, 15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 120, 100
    WAIT

    SPEED 3
    MOVE G6A, 65, 157,  40, 162, 155, 100
    MOVE G6D, 65, 157,  40, 162, 155, 100
    MOVE G6B,190,  25, 80, 100, 160, 100
    MOVE G6C,190,  25, 80, 100, 120, 100
    WAIT

    '****************************************************************
    SPEED 5
    MOVE G6A, 60, 162,  30, 162, 145, 100
    MOVE G6D, 60, 162,  30, 162, 145, 100
    MOVE G6B,190,  25, 80, 100, 160, 100
    MOVE G6C,190,  25, 80, 100, 120, 100
    WAIT

    SPEED 5
    MOVE G6A, 60, 150,  28, 155, 140, 100
    MOVE G6D, 60, 150,  28, 155, 140, 100
    MOVE G6B,190,  25, 80, 100, 160, 100
    MOVE G6C,190,  25, 80, 100, 120, 100
    WAIT

    SPEED 5
    MOVE G6A,100, 150,  28, 140, 100, 100
    MOVE G6D,100, 150,  28, 140, 100, 100
    MOVE G6B,190,  25, 80, 100, 160, 100
    MOVE G6C,190,  25, 80, 100, 120, 100
    WAIT
    DELAY 100


    SPEED 7
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,190,  25, 80, 100, 160, 100
    MOVE G6C,190,  25, 80, 100, 120, 100
    WAIT

    SPEED 7
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 160, 100
    MOVE G6C,100,  35,  80, 100, 120, 100
    WAIT

    DELAY 100
    RETURN
    '**************************************************************
SWalkback: '이거 고치던지 지우던지 해야돼!!!!

    GOSUB 자이로OFF

    GOSUB Leg_motor_mode2
    SPEED 10
    MOVE G6A,100,  76, 149,  93,  100, 100
    MOVE G6D,100,  76, 138,  81, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    MOVE G6A,100,  76, 138,  81, 100, 100
    MOVE G6D,100,  76, 149, 93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    MOVE G6A,100,  76, 149, 93, 100, 100
    MOVE G6D,100,  76, 138,  81, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    MOVE G6A,100,  76, 138,  81, 100, 100
    MOVE G6D,100,  76, 149, 94, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT


    SPEED 6
    MOVE G6A,100,  76, 149, 93, 100, 100
    MOVE G6D,100,  76, 138,  81, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    MOVE G6A,100,  76, 138,  81, 100, 100
    MOVE G6D,100,  76, 148, 93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    MOVE G6A,100,  76, 149, 93, 100, 100
    MOVE G6D,100,  76, 138,  81, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    MOVE G6A,100,  76, 138,  81, 100, 100
    MOVE G6D,100,  76, 149, 93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    MOVE G6A,100,  76, 142,  88, 100, 100
    MOVE G6D,100,  76, 149, 94, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    GOSUB 기본자세
    GOSUB 자이로ON

    RETURN

    '***********************************************
SWalk:

    GOSUB 자이로OFF

    GOSUB Leg_motor_mode2

    SPEED 10
    MOVE G6A,100,  76, 143, 102, 100, 100
    MOVE G6D,100,  76, 145,  92, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    MOVE G6A,100,  76, 143,  92, 100, 100
    MOVE G6D,100,  76, 145, 102, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    MOVE G6A,100,  76, 143, 102, 100, 100
    MOVE G6D,100,  76, 145,  92, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    MOVE G6A,100,  76, 143,  92, 100, 100
    MOVE G6D,100,  76, 145, 102, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT


    SPEED 6
    MOVE G6A,100,  76, 143, 102, 100, 100
    MOVE G6D,100,  76, 145,  92, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    MOVE G6A,100,  76, 143,  92, 100, 100
    MOVE G6D,100,  76, 145, 102, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    MOVE G6A,100,  76, 143, 102, 100, 100
    MOVE G6D,100,  76, 145,  92, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    MOVE G6A,100,  76, 143,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 25
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 117, 100
    WAIT


    GOSUB 기본자세
    GOSUB 자이로ON
    '3.5번 줄임
    RETURN
    '***********************************************
    '***********************************************

SWalk2: 'hyun각도로 왼	쪽

    GOSUB 자이로OFF

    GOSUB Leg_motor_mode2
    SPEED 10
    MOVE G6A,100,  76, 143, 102, 100, 100
    MOVE G6D,100,  76, 145,  92, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT

    MOVE G6A,100,  76, 143,  92, 100, 100
    MOVE G6D,100,  76, 145, 102, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT

    MOVE G6A,100,  76, 143, 102, 100, 100
    MOVE G6D,100,  76, 145,  92, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT

    MOVE G6A,100,  76, 143,  92, 100, 100
    MOVE G6D,100,  76, 145, 102, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT


    SPEED 6
    MOVE G6A,100,  76, 143, 102, 100, 100
    MOVE G6D,100,  76, 145,  92, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT

    MOVE G6A,100,  76, 143,  92, 100, 100
    MOVE G6D,100,  76, 145, 102, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT

    MOVE G6A,100,  76, 143, 102, 100, 100
    MOVE G6D,100,  76, 145,  92, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT

    MOVE G6A,100,  76, 143,  92, 100, 100
    MOVE G6D,100,  76, 145, 102, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT

    MOVE G6A,100,  76, 143,  93, 100, 100
    MOVE G6D,100,  76, 145, 93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT

    GOSUB 자이로ON
    '3.5번 줄임
    RETURN
    '***********************************
    '***********************************
SWalk3: 'kyeong각도로 걷기

    GOSUB 자이로OFF

    GOSUB Leg_motor_mode2

    SPEED 10
    MOVE G6A,100,  76, 143, 102, 100, 100
    MOVE G6D,100,  76, 145,  92, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT

    MOVE G6A,100,  76, 143,  92, 100, 100
    MOVE G6D,100,  76, 145, 102, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT

    MOVE G6A,100,  76, 143, 102, 100, 100
    MOVE G6D,100,  76, 145,  92, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT

    MOVE G6A,100,  76, 143,  92, 100, 100
    MOVE G6D,100,  76, 145, 102, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT


    SPEED 6
    MOVE G6A,100,  76, 143, 102, 100, 100
    MOVE G6D,100,  76, 145,  92, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT

    MOVE G6A,100,  76, 143,  92, 100, 100
    MOVE G6D,100,  76, 145, 102, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT

    MOVE G6A,100,  76, 143, 102, 100, 100
    MOVE G6D,100,  76, 145,  92, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT

    MOVE G6A,100,  76, 143,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 25
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT

    MOVE G6A,100,  76, 143,  93, 100, 100
    MOVE G6D,100,  76, 145, 93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT

    GOSUB 자이로ON
    '3.5번 줄임
    RETURN
    '***********************************

    '**********************************************
armheaddown:

    SPEED 7
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,165, 15,  55, 100, 160, 100
    MOVE G6C,165,  15,  55, 100, 120, 100
    WAIT

    DELAY 100
    RETURN
    '****************************************************************
backstepleft: '뒷걸음질 원본

    GOSUB 자이로ON
    HIGHSPEED SETOFF
    GOSUB Leg_motor_mode3

    '오른쪽기울기
    SPEED 4
    MOVE G6A, 88,  71, 152,  91, 110
    MOVE G6D,108,  76, 146,  93,  94
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    '왼발들기
    SPEED 10
    MOVE G6A, 90, 95, 115, 105, 114
    MOVE G6D,113,  78, 146,  93,  94
    MOVE G6B,90,  35,  80, 100, 100, 100
    MOVE G6C,110,  35,  80, 100, 110, 100
    WAIT
    '*************************************************
    GOSUB Leg_motor_mode2
    '오른발중심이동
    SPEED 10
    MOVE G6D,110,  76, 144, 100,  93
    MOVE G6A, 90, 93, 155,  71, 112
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    GOSUB Leg_motor_mode3
    '오른발뻣어착지
    SPEED 4
    MOVE G6D,90,  46, 163, 110, 114
    MOVE G6A,110,  77, 147,  90,  94
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    '오른발들기10
    SPEED 10
    MOVE G6A,112,  77, 147,  93, 94
    MOVE G6D,90, 100, 105, 110, 114
    MOVE G6B,110,  35,  80, 100, 100, 100
    MOVE G6C,90,  35,  80, 100, 110, 100
    WAIT

    GOSUB Leg_motor_mode3

    '왼쪽기울기2
    SPEED 5
    MOVE G6A, 106,  76, 146,  93,  96		
    MOVE G6D,  88,  71, 152,  91, 106
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT	


    SPEED 5
    GOSUB 기본자세

    DELAY 100

    RETURN

    '************************************************

    '************************************************
backstepleft2: 'hyun각도로 숙이고 앞쪽

    GOSUB 자이로ON
    HIGHSPEED SETOFF
    GOSUB Leg_motor_mode3

    '오른쪽기울기
    SPEED 4
    MOVE G6A, 88,  71, 152,  91, 110
    MOVE G6D,108,  76, 146,  93,  94
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT

    '왼발들기
    SPEED 10
    MOVE G6A, 90, 95, 115, 105, 114
    MOVE G6D,113,  78, 146,  93,  94
    MOVE G6B,90,   35,  80, 100, 100 ,100
    MOVE G6C,110,  35,  80, 100, 140, 100
    WAIT
    '*************************************************
    GOSUB Leg_motor_mode2
    '오른발중심이동
    SPEED 10
    MOVE G6D,110,  76, 144, 100,  93
    MOVE G6A, 90, 93, 155,  71, 112
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT

    GOSUB Leg_motor_mode3
    '오른발뻣어착지
    SPEED 4
    MOVE G6D,90,  46, 163, 110, 114
    MOVE G6A,110,  77, 147,  90,  94
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT

    '오른발들기10
    SPEED 10
    MOVE G6A,112,  77, 147,  93, 94
    MOVE G6D,90, 100, 105, 110, 114
    MOVE G6B,110, 35, 80,  100, 100, 100
    MOVE G6C,90,  35, 80,  100, 140, 100
    WAIT

    GOSUB Leg_motor_mode3

    '왼쪽기울기2
    SPEED 5
    MOVE G6A, 106,  76, 146,  93,  96		
    MOVE G6D,  88,  71, 152,  91, 106
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT	

    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 25
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT
    DELAY 100

    RETURN

    '************************************************
    '************************************************
backstepleft3: 'kyeong각도로 숙이고 정면

    GOSUB 자이로ON
    HIGHSPEED SETOFF
    GOSUB Leg_motor_mode3

    '오른쪽기울기
    SPEED 4
    MOVE G6A, 88,  71, 152,  91, 110
    MOVE G6D,108,  76, 146,  93,  94
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT

    '왼발들기
    SPEED 10
    MOVE G6A, 90, 95, 115, 105, 114
    MOVE G6D,113,  78, 146,  93,  94
    MOVE G6B,90,   35,  80, 100, 100 ,100
    MOVE G6C,110,  35,  80, 100, 170, 100
    WAIT
    '*************************************************
    GOSUB Leg_motor_mode2
    '오른발중심이동
    SPEED 10
    MOVE G6D,110,  76, 144, 100,  93
    MOVE G6A, 90, 93, 155,  71, 112
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT

    GOSUB Leg_motor_mode3
    '오른발뻣어착지
    SPEED 4
    MOVE G6D,90,  46, 163, 110, 114
    MOVE G6A,110,  77, 147,  90,  94
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT

    '오른발들기10
    SPEED 10
    MOVE G6A,112,  77, 147,  93, 94
    MOVE G6D,90, 100, 105, 110, 114
    MOVE G6B,110, 35, 80,  100, 100, 100
    MOVE G6C,90,  35, 80,  100, 170, 100
    WAIT

    GOSUB Leg_motor_mode3

    '왼쪽기울기2
    SPEED 5
    MOVE G6A, 106,  76, 146,  93,  96		
    MOVE G6D,  88,  71, 152,  91, 106
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT	

    SPEED 5
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 25
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT
    DELAY 100

    RETURN

    '************************************************
dooropen:

    GOSUB 자이로ON
    HIGHSPEED SETOFF
    GOSUB All_motor_mode3

    SPEED 10
    MOVE G6A,100,  76, 145,  95, 100, 100
    MOVE G6D,100,  76, 145,  95, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 117, 100
    WAIT

    SPEED 10
    MOVE G6A,100,  76, 145,  95, 100, 100
    MOVE G6D,100,  76, 145,  95, 100, 100
    MOVE G6B,180,  35,  80, 100, 100, 100
    MOVE G6C,180,  35,  80, 100, 117, 100
    WAIT

    SPEED 10
    MOVE G6D,108,  76, 145,  80, 104, 100
    MOVE G6A,100,  86, 125, 109, 100, 100
    MOVE G6B,180,  10,  55, 100, 100, 100
    MOVE G6C,180,  10,  55, 100, 117, 100
    WAIT


    '****************************************

   FOR I = 0 TO 10

        SPEED 10
        MOVE G6A, 99,  66, 145, 103, 100, 100
        MOVE G6D,100,  86, 145,  83, 100, 100
        MOVE G6B,180,  10,  55, 100, 100, 100
        MOVE G6C,180,  10,  55, 100, 117, 100
        WAIT

        SPEED 10
        MOVE G6A,108,  76, 145,  93, 104, 100
        MOVE G6D,100,  86, 125, 103, 100, 100
        MOVE G6B,180,  10,  55, 100, 100, 100
        MOVE G6C,180,  10,  55, 100, 117, 100
        WAIT

        SPEED 10
        MOVE G6D,101,  66, 145, 103, 100, 100
        MOVE G6A,100,  86, 145,  83, 100, 100
        MOVE G6B,180,  10,  55, 100, 100, 100
        MOVE G6C,180,  10,  55, 100, 117, 100
        WAIT

        SPEED 10
        MOVE G6D,108,  76, 145,  93, 104, 100
        MOVE G6A,100,  86, 125, 103, 100, 100
        MOVE G6B,180,  10,  55, 100, 100, 100
        MOVE G6C,180,  10,  55, 100, 117, 100
        WAIT

    NEXT I

    '****************************************

    SPEED 10
    MOVE G6A, 99,  66, 145, 103, 100, 100
    MOVE G6D,100,  86, 145,  83, 100, 100
    MOVE G6B,180,  10,  55, 100, 100, 100
    MOVE G6C,180,  10,  55, 100, 117, 100
    WAIT

    SPEED 10
    MOVE G6A,108,  76, 145,  93, 104, 100
    MOVE G6D,100,  86, 125, 103, 100, 100
    MOVE G6B,180,  10,  55, 100, 100, 100
    MOVE G6C,180,  10,  55, 100, 117, 100
    WAIT

    SPEED 10
    MOVE G6D,101,  66, 145, 103, 100, 100
    MOVE G6A,100,  86, 145,  83, 100, 100
    MOVE G6B,180,  10,  55, 100, 100, 100
    MOVE G6C,180,  10,  55, 100, 117, 100
    WAIT

    SPEED 10
    MOVE G6D,108,  76, 145,  91, 104, 100
    MOVE G6A,100,  86, 125,  97, 100, 100
    MOVE G6B,180,  10,  55, 100, 100, 100
    MOVE G6C,180,  10,  55, 100, 117, 100
    WAIT

    '****************************************

    SPEED 10
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,180,  10,  55, 100, 100, 100
    MOVE G6C,180,  10,  55, 100, 117, 100
    WAIT

    '***************'''''''''''''''''''''''''    '뒷걸음질 반복
    FOR I=0 TO 2

        '오른쪽기울기
        SPEED 4
        MOVE G6A, 88,  71, 152,  91, 110
        MOVE G6D,108,  76, 146,  93,  94
        MOVE G6B,100,50
        MOVE G6C,100,50
        WAIT

        '왼발들기
        SPEED 10
        MOVE G6A, 90, 95, 115, 105, 114
        MOVE G6D,113,  78, 146,  93,  94
        MOVE G6B,100,50
        MOVE G6C,100,50
        WAIT
        '*************************************************
        GOSUB Leg_motor_mode2
        '오른발중심이동
        SPEED 10
        MOVE G6D,110,  76, 144, 100,  93
        MOVE G6A, 90, 93, 155,  71, 112
        MOVE G6B,100,50
        MOVE G6C,100,50
        WAIT

        GOSUB Leg_motor_mode3
        '오른발뻣어착지
        SPEED 4
        MOVE G6D,90,  46, 163, 110, 114
        MOVE G6A,110,  77, 147,  90,  94
        MOVE G6B,100,50
        MOVE G6C,100,50
        WAIT

        '오른발들기10
        SPEED 10
        MOVE G6A,112,  77, 147,  93, 94
        MOVE G6D,90, 100, 105, 110, 114
        MOVE G6B,100,50
        MOVE G6C,100,50
        WAIT

        GOSUB Leg_motor_mode3

        '왼쪽기울기2
        SPEED 5
        MOVE G6A, 106,  76, 146,  93,  96      
        MOVE G6D,  88,  71, 152,  91, 106
        MOVE G6B, 100,50
        MOVE G6C, 100,50
        WAIT   

    NEXT I
    '****************''''''''''''''''''''''''''''''''''


    SPEED 5
    GOSUB 기본자세

    DELAY 100

    RETURN
    '************************************************

    '************************************************
backstepright:

    GOSUB 자이로ON
    HIGHSPEED SETOFF
    GOSUB Leg_motor_mode3


    SPEED 4
    MOVE G6D, 88,  71, 152,  91, 110
    MOVE G6A,108,  76, 146,  93,  94
    MOVE G6B,100,35
    MOVE G6C,100,35
    WAIT


    SPEED 10
    MOVE G6D, 90, 95, 115, 105, 114
    MOVE G6A,113,  78, 146,  93,  94
    MOVE G6B,90
    MOVE G6C,110
    WAIT
    '*************************************************
    GOSUB Leg_motor_mode2

    SPEED 10
    MOVE G6A,110,  76, 144, 100,  93
    MOVE G6D, 90, 93, 155,  71, 112
    WAIT

    GOSUB Leg_motor_mode3

    SPEED 4
    MOVE G6A,90,  46, 163, 110, 114
    MOVE G6D,110,  77, 147,  90,  94
    WAIT


    SPEED 10
    MOVE G6D,112,  77, 147,  93, 94
    MOVE G6A,90, 100, 105, 110, 114
    MOVE G6B,110
    MOVE G6C,90
    WAIT

    GOSUB Leg_motor_mode3


    SPEED 5
    MOVE G6D, 106,  76, 146,  93,  96		
    MOVE G6A,  88,  71, 152,  91, 106
    MOVE G6B, 100,35
    MOVE G6C, 100,35
    WAIT	

    SPEED 7
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 25
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT


    DELAY 100

    RETURN

    '************************************************
GO_FRONT2:

    GOSUB 자이로ON

    GOSUB All_motor_mode3

    SPEED 4
    '왼쪽기울기
    MOVE G6D, 90,  73, 152,  88, 105
    MOVE G6A,104,  79, 146,  90, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    WAIT

    SPEED 12'보행속도
    '오른발 들기
    MOVE G6D, 83, 98, 115, 104, 111
    MOVE G6A,108,  84, 140,  90,  96
    MOVE G6C,90,  35,  80, 100, 110, 100
    MOVE G6B,110,  35,  80, 100, 100, 100
    WAIT

    SPEED 13
    '오른발뻣어착지
    MOVE G6D, 94,  53, 153, 112,  102
    MOVE G6A,106,  84, 142,  90,  100
    MOVE G6C,100,  35,  80, 100, 110, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    WAIT

    SPEED 5
    '오른발 중심이동
    MOVE G6D,103,  74, 137, 104,  105
    MOVE G6A,95, 88, 150,  75, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    WAIT

    SPEED 7
    '왼발들기10
    MOVE G6D,106,  75, 145,  96, 98
    MOVE G6A,95, 87, 118, 110, 103
    MOVE G6C,110,  35,  80, 100, 110, 100
    MOVE G6B,90,  35,  80, 100, 100, 100
    WAIT

    SPEED 5
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    WAIT

    GOSUB 자이로OFF

    RETURN
    '************************************************
GO_FRONT22: 'hyun각도로 한발자국

    GOSUB 자이로ON

    GOSUB All_motor_mode3

    SPEED 4
    '왼쪽기울기
    MOVE G6D, 90,  73, 152,  88, 105
    MOVE G6A,104,  79, 146,  90, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    WAIT

    SPEED 12'보행속도
    '오른발 들기
    MOVE G6D, 83, 98, 115, 104, 111
    MOVE G6A,108,  84, 140,  90,  96
    MOVE G6C,90,  35,  80, 100, 140, 100
    MOVE G6B,110, 35,  80, 100, 100, 100
    WAIT

    SPEED 13
    '오른발뻣어착지
    MOVE G6D, 94,  53, 153, 112,  102
    MOVE G6A,106,  84, 142,  90,  100
    MOVE G6C,100,  35,  80, 100, 140, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    WAIT

    SPEED 5
    '오른발 중심이동
    MOVE G6D,103,  74, 137, 104,  105
    MOVE G6A,95, 88, 150,  75, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    WAIT

    SPEED 7
    '왼발들기10
    MOVE G6D,106,  75, 145,  96, 98
    MOVE G6A,95, 87, 118, 110, 103
    MOVE G6C,110,  35,  80, 100, 140, 100
    MOVE G6B,90,  35,  80, 100, 100, 100
    WAIT

    SPEED 5
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    WAIT

    GOSUB 자이로OFF

    RETURN
    '************************************************
GO_FRONT23: 'kyeong각도로 한발자국

    GOSUB 자이로ON

    GOSUB All_motor_mode3

    SPEED 4
    '왼쪽기울기
    MOVE G6D, 90,  73, 152,  88, 105
    MOVE G6A,104,  79, 146,  90, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    WAIT

    SPEED 12'보행속도
    '오른발 들기
    MOVE G6D, 83, 98, 115, 104, 111
    MOVE G6A,108,  84, 140,  90,  96
    MOVE G6C,90,  35,  80, 100, 170, 100
    MOVE G6B,110, 35,  80, 100, 100, 100
    WAIT

    SPEED 13
    '오른발뻣어착지
    MOVE G6D, 94,  53, 153, 112,  102
    MOVE G6A,106,  84, 142,  90,  100
    MOVE G6C,100,  35,  80, 100, 170, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    WAIT

    SPEED 5
    '오른발 중심이동
    MOVE G6D,103,  74, 137, 104,  105
    MOVE G6A,95, 88, 150,  75, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    WAIT

    SPEED 7
    '왼발들기10
    MOVE G6D,106,  75, 145,  96, 98
    MOVE G6A,95, 87, 118, 110, 103
    MOVE G6C,110,  35,  80, 100, 170, 100
    MOVE G6B,90,  35,  80, 100, 100, 100
    WAIT

    SPEED 5
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    WAIT

    GOSUB 자이로OFF

    RETURN
    '************************************************
kilfront:
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT
    mode=0
    RETURN
    '************************************************
kilfront2:
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 100, 100
    WAIT
    mode=0
    RETURN
    '************************************************
killeft:

    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 60
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT
    mode=0
    RETURN
    '*********************************************
kilright:

    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 140
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT
    mode=0
    RETURN
    '***********************************************

hyunleft: '나중에 다리부분 수정
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 60
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT
    mode = 0
    RETURN
    '*********************************************
hyunright:
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 140
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT
    mode = 0
    RETURN
    '**********************************************
kyeongleft: '머리완전 팍 숙인채로 45도 왼쪽
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 60
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT
    mode = 0
    RETURN
    '*********************************************
kyeongright:
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 140
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT
    mode = 0
    RETURN
    '*********************************************
hyunfront:
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT
    mode = 0
    RETURN
    '*********************************************
kyeongfront:
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT
    mode = 0
    RETURN
    '************************************************
headright:
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 25
    MOVE G6B,100,  35,  80, 100, 100, 115
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT
    mode = 0

    RETURN
    '*********************************************
milkwalkleft:

    SPEED 10
    MOVE G6A, 90,  90, 120, 105, 110, 100
    MOVE G6D,100,  76, 146,  93, 107, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT

    SPEED 12
    MOVE G6A, 102,  76, 147, 93, 100, 100
    MOVE G6D,83,  78, 140,  96, 115, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT

    SPEED 10
    MOVE G6A,98,  76, 146,  93, 100, 100
    MOVE G6D,98,  76, 146,  93, 100, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT


    GOTO MAIN
    '**************************************************
milkwalkright:
    SPEED 10
    MOVE G6D, 90,  90, 120, 105, 110, 100
    MOVE G6A,100,  76, 146,  93, 107, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT

    SPEED 12
    MOVE G6D, 102,  76, 147, 93, 100, 100
    MOVE G6A,83,  78, 140,  96, 115, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT

    SPEED 10
    MOVE G6D,98,  76, 146,  93, 100, 100
    MOVE G6A,98,  76, 146,  93, 100, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT



    GOTO MAIN
    '************************************************
milkSWalk:

    GOSUB 자이로OFF

    GOSUB Leg_motor_mode2
    SPEED 10
    MOVE G6A,100,  76, 145, 102, 100, 100
    MOVE G6D,100,  76, 145,  92, 100, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100

    MOVE G6A,100,  76, 145,  92, 100, 100
    MOVE G6D,100,  76, 145, 102, 100, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT

    MOVE G6A,100,  76, 145, 102, 100, 100
    MOVE G6D,100,  76, 145,  92, 100, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT

    MOVE G6A,100,  76, 145,  92, 100, 100
    MOVE G6D,100,  76, 145, 102, 100, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT


    SPEED 6
    MOVE G6A,100,  76, 145, 102, 100, 100
    MOVE G6D,100,  76, 145,  92, 100, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT

    MOVE G6A,100,  76, 145,  92, 100, 100
    MOVE G6D,100,  76, 145, 102, 100, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT

    MOVE G6A,100,  76, 145, 102, 100, 100
    MOVE G6D,100,  76, 145,  92, 100, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT

    MOVE G6A,100,  76, 145,  92, 100, 100
    MOVE G6D,100,  76, 145, 102, 100, 100
    MOVE G6B,190,  15,  55, 100, 160, 100
    MOVE G6C,190,  15,  55, 100, 160, 100
    WAIT

    GOSUB 자이로ON
    '3.5번 줄임
    RETURN
    '*****************************************************
woosturnleft: 'JLturn10 수정한것

    GOSUB 자이로OFF

    GOSUB Leg_motor_mode1

    SPEED 15
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6A,100,  76, 145,  93, 98, 100
    WAIT

    SPEED 8
    MOVE G6D,100,  61, 145,  93, 100, 100
    MOVE G6A,100,  86, 155,  88, 100, 100
    WAIT


    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6A,100,  76, 145,  93, 100, 100
    WAIT

    DELAY 100



    RETURN
    '***************************************************
TurnLeft60:
    SPEED 14
    MOVE G6A,95,  116, 145,  53, 105, 100
    MOVE G6D,95,  36, 145,  133, 105, 100
    MOVE G6B,100
    MOVE G6C,100, , , ,110
    WAIT

    SPEED 14
    MOVE G6A,90,  116, 145,  53, 105, 100
    MOVE G6D,90,  36, 145,  133, 105, 100
    MOVE G6B,100
    MOVE G6C,100, , , ,110
    WAIT

    SPEED 9
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100
    MOVE G6C,100, , , ,110

    WAIT

    RETURN
    '***************************************************
TurnLeft603:

    SPEED 14
    MOVE G6A,95,  116, 145,  53, 105, 100
    MOVE G6D,95,  36, 145,  133, 105, 100
    MOVE G6B,100
    MOVE G6C,100, , , ,170
    WAIT

    SPEED 14
    MOVE G6A,90,  116, 145,  53, 105, 100
    MOVE G6D,90,  36, 145,  133, 105, 100
    MOVE G6B,100
    MOVE G6C,100, , , ,170
    WAIT

    SPEED 9
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100
    MOVE G6C,100, , , ,170

    WAIT

    RETURN
    '***************************************************
MTurnLeft60:

    SPEED 14
    MOVE G6A,95,  116, 145,  53, 105, 100
    MOVE G6D,95,  36, 145,  133, 105, 100
    MOVE G6B,190,  15,  55, 100, 170, 100
    MOVE G6C,190,  15,  55, 100, 170, 100
    WAIT

    SPEED 14
    MOVE G6A,90,  116, 145,  53, 105, 100
    MOVE G6D,90,  36, 145,  133, 105, 100
    MOVE G6B,190,  15,  55, 100, 170, 100
    MOVE G6C,190,  15,  55, 100, 170, 100
    WAIT

    SPEED 9
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,190,  15,  55, 100, 170, 100
    MOVE G6C,190,  15,  55, 100, 170, 100

    WAIT

    RETURN
    '***************************************************
TurnRight60:

    SPEED 14
    MOVE G6A,95,  36, 145,  133, 105, 100
    MOVE G6D,95,  116, 145,  53, 105, 100
    MOVE G6B,100
    MOVE G6C,100, , , ,110
    WAIT

    SPEED 14
    MOVE G6A,90,  36, 145,  133, 105, 100
    MOVE G6D,90,  116, 145,  53, 105, 100
    MOVE G6B,100
    MOVE G6C,100, , , ,110
    WAIT

    SPEED 9
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100
    MOVE G6C,100, , , ,110

    WAIT

    RETURN
    '***************************************************
TurnRight603:

    SPEED 14
    MOVE G6A,95,  36, 145,  133, 105, 100
    MOVE G6D,95,  116, 145,  53, 105, 100
    MOVE G6B,100
    MOVE G6C,100, , , ,170
    WAIT

    SPEED 14
    MOVE G6A,90,  36, 145,  133, 105, 100
    MOVE G6D,90,  116, 145,  53, 105, 100
    MOVE G6B,100
    MOVE G6C,100, , , ,170


    SPEED 9
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100
    MOVE G6C,100, , , ,170

    WAIT

    RETURN
    '***************************************************
MTurnRight60:

    SPEED 14
    MOVE G6A,95,  36, 145,  133, 105, 100
    MOVE G6D,95,  116, 145,  53, 105, 100
    MOVE G6B,190,  15,  55, 100, 170, 100
    MOVE G6C,190,  15,  55, 100, 170, 100
    WAIT

    SPEED 14
    MOVE G6A,90,  36, 145,  133, 105, 100
    MOVE G6D,90,  116, 145,  53, 105, 100
    MOVE G6B,190,  15,  55, 100, 170, 100
    MOVE G6C,190,  15,  55, 100, 170, 100

    SPEED 9
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,190,  15,  55, 100, 170, 100
    MOVE G6C,190,  15,  55, 100, 170, 100

    WAIT

    RETURN
    '***************************************
왼쪽턴180_LOOP_4:

    GOSUB 자이로ON
    HIGHSPEED SETOFF
    GOSUB Leg_motor_mode6


    FOR I=0 TO 3

        SPEED 15
        MOVE G6A,95,  116, 145,  53, 105, 100
        MOVE G6D,95,  36, 145,  133, 105, 100
        WAIT

        SPEED 15
        MOVE G6A,90,  116, 145,  53, 105, 100
        MOVE G6D,90,  36, 145,  133, 105, 100
        WAIT

        SPEED 10
        GOSUB 기본자세2
        DELAY 50

    NEXT I

    RETURN

    '*******************************
ML180_LOOP_4:

    GOSUB 자이로ON
    HIGHSPEED SETOFF
    GOSUB Leg_motor_mode6


    FOR I=0 TO 3

        SPEED 15
        MOVE G6A,95,  116, 145,  53, 105, 100
        MOVE G6D,95,  36, 145,  133, 105, 100
        MOVE G6B,190,  15,  55, 100, 160, 100
        MOVE G6C,190,  15,  55, 100, 160, 100        
        WAIT

        SPEED 15
        MOVE G6A,90,  116, 145,  53, 105, 100
        MOVE G6D,90,  36, 145,  133, 105, 100
        MOVE G6B,190,  15,  55, 100, 160, 100
        MOVE G6C,190,  15,  55, 100, 160, 100        
        WAIT
 
        SPEED 15
 	    MOVE G6A,100,  76, 145,  93, 100, 100
  	    MOVE G6D,100,  76, 145,  93, 100, 100
        MOVE G6B,190,  15,  55, 100, 160, 100
        MOVE G6C,190,  15,  55, 100, 160, 100        
        WAIT
        
    NEXT I

    RETURN
    '***************************    
왼쪽턴180_LOOP_3:

    GOSUB 자이로ON
    HIGHSPEED SETOFF
    GOSUB Leg_motor_mode6


    FOR I=0 TO 2

        SPEED 15
        MOVE G6A,95,  116, 145,  53, 105, 100
        MOVE G6D,95,  36, 145,  133, 105, 100
        WAIT

        SPEED 15
        MOVE G6A,90,  116, 145,  53, 105, 100
        MOVE G6D,90,  36, 145,  133, 105, 100
        WAIT

        SPEED 10
        GOSUB 기본자세2
        DELAY 50

    NEXT I


    RETURN

    '*********************************
오른쪽턴100_LOOP:

    GOSUB 자이로ON
    HIGHSPEED SETOFF
    GOSUB Leg_motor_mode6

    FOR I=0 TO 2

        SPEED 15
        MOVE G6A,95,  36, 145,  133, 105, 100
        MOVE G6D,95,  116, 145,  53, 105, 100
        WAIT

        SPEED 15
        MOVE G6A,90,  36, 145,  133, 105, 100
        MOVE G6D,90,  116, 145,  53, 105, 100
        WAIT

        SPEED 10
        GOSUB 기본자세2
        DELAY 50

    NEXT I

    RETURN
    '********************************
Lastdooropen:

    GOSUB 자이로ON
    HIGHSPEED SETOFF
    GOSUB All_motor_mode3

    SPEED 10
    MOVE G6A,100,  76, 145,  95, 100, 100
    MOVE G6D,100,  76, 145,  95, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 117, 100
    WAIT

    SPEED 10
    MOVE G6A,100,  76, 145,  95, 100, 100
    MOVE G6D,100,  76, 145,  95, 100, 100
    MOVE G6B,180,  35,  80, 100, 100, 100
    MOVE G6C,180,  35,  80, 100, 117, 100
    WAIT

    SPEED 10
    MOVE G6D,108,  76, 145,  80, 104, 100
    MOVE G6A,100,  86, 125, 109, 100, 100
    MOVE G6B,180,  10,  50, 100, 100, 100
    MOVE G6C,180,  10,  60, 100, 117, 100
    WAIT


    '****************************************

    FOR I = 0 TO 7

        SPEED 10
        MOVE G6A, 99,  66, 145, 103, 100, 100
        MOVE G6D,100,  86, 145,  83, 100, 100
        MOVE G6B,180,  10,  50, 100, 100, 100
        MOVE G6C,180,  10,  60, 100, 117, 100
        WAIT

        SPEED 10
        MOVE G6A,108,  76, 145,  93, 104, 100
        MOVE G6D,100,  86, 125, 103, 100, 100
        MOVE G6B,180,  10,  50, 100, 100, 100
        MOVE G6C,180,  10,  60, 100, 117, 100
        WAIT

        SPEED 10
        MOVE G6D,101,  66, 145, 103, 100, 100
        MOVE G6A,100,  86, 145,  83, 100, 100
        MOVE G6B,180,  10,  50, 100, 100, 100
        MOVE G6C,180,  10,  60, 100, 117, 100
        WAIT

        SPEED 10
        MOVE G6D,108,  76, 145,  93, 104, 100
        MOVE G6A,100,  86, 125, 103, 100, 100
        MOVE G6B,180,  10,  50, 100, 100, 100
        MOVE G6C,180,  10,  60, 100, 117, 100
        WAIT

    NEXT I

    '****************************************

    SPEED 10
    MOVE G6A, 99,  66, 145, 103, 100, 100
    MOVE G6D,100,  86, 145,  83, 100, 100
    MOVE G6B,180,  10,  50, 100, 100, 100
    MOVE G6C,180,  10,  60, 100, 117, 100
    WAIT

    SPEED 10
    MOVE G6A,108,  76, 145,  93, 104, 100
    MOVE G6D,100,  86, 125, 103, 100, 100
    MOVE G6B,180,  10,  50, 100, 100, 100
    MOVE G6C,180,  10,  60, 100, 117, 100
    WAIT

    SPEED 10
    MOVE G6D,101,  66, 145, 103, 100, 100
    MOVE G6A,100,  86, 145,  83, 100, 100
    MOVE G6B,180,  10,  50, 100, 100, 100
    MOVE G6C,180,  10,  60, 100, 117, 100
    WAIT

    SPEED 10
    MOVE G6D,108,  76, 145,  91, 104, 100
    MOVE G6A,100,  86, 125,  97, 100, 100
    MOVE G6B,180,  10,  50, 100, 100, 100
    MOVE G6C,180,  10,  60, 100, 117, 100
    WAIT

    '****************************************

    SPEED 10
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,180,  10,  50, 100, 100, 100
    MOVE G6C,180,  10,  60, 100, 117, 100
    WAIT

    SPEED 5
    GOSUB 기본자세

    DELAY 100

    RETURN

    '**********************************************
Lwalk:
    GOSUB 자이로OFF

    GOSUB Leg_motor_mode2

    SPEED 11
    MOVE G6A, 93,  88, 125, 104, 104, 100
    MOVE G6D,99,  76, 145,  96, 104, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    SPEED 11
    MOVE G6A, 102,  78, 145, 94, 100, 100
    MOVE G6D,90,  80, 140,  96, 107, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    SPEED 11
    MOVE G6A,98,  76, 145,  93, 102, 100
    MOVE G6D,98,  76, 145,  93, 102, 100
    MOVE G6B,100,  35,  80, 100, 100, 100	
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    SPEED 8
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    RETURN
    '**********************************************
LWALK5:

    SPEED 10
    MOVE G6A, 90,  93, 120, 105, 110, 100
    MOVE G6D,100,  73, 145,  93, 107, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100    
    WAIT

    SPEED 10
    MOVE G6A, 102,  79, 145, 93, 100, 100
    MOVE G6D,83,  75, 140,  96, 115, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100    
    WAIT

    SPEED 10
    MOVE G6A,98,  79, 145,  93, 100, 100
    MOVE G6D,98,  73, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100    
    WAIT

    SPEED 9
    MOVE G6D,100,  73, 145,  93, 100, 100
    MOVE G6A,100,  79, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100    
    WAIT
    
    SPEED 8
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    RETURN
    '*************************************************
KLWALK5:

    SPEED 10
    MOVE G6A, 90,  93, 120, 105, 110, 100
    MOVE G6D,100,  73, 145,  93, 107, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100    
    WAIT

    SPEED 10
    MOVE G6A, 102,  79, 145, 93, 100, 100
    MOVE G6D,83,  75, 140,  96, 115, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100    
    WAIT

    SPEED 10
    MOVE G6A,98,  79, 145,  93, 100, 100
    MOVE G6D,98,  73, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100    
    WAIT

    SPEED 9
    MOVE G6D,100,  73, 145,  93, 100, 100
    MOVE G6A,100,  79, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100    
    WAIT

    RETURN
    '*************************************************    
RWALK5:

    SPEED 10
    MOVE G6D, 90,  93, 120, 105, 110, 100
    MOVE G6A,100,  73, 145,  93, 107, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100    
    WAIT

    SPEED 10
    MOVE G6D, 102,  79, 145, 93, 100, 100
    MOVE G6A,83,  75, 140,  96, 115, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100    
    WAIT

    SPEED 10
    MOVE G6D,98,  79, 145,  93, 100, 100
    MOVE G6A,98,  73, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100    
    WAIT

    SPEED 9
    MOVE G6D,100,  73, 145,  93, 100, 100
    MOVE G6A,100,  79, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100    
    WAIT

    SPEED 8
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    RETURN
    '***********************************************  
KRWALK5:

    SPEED 10
    MOVE G6D, 90,  93, 120, 105, 110, 100
    MOVE G6A,100,  73, 145,  93, 107, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100        
    WAIT

    SPEED 10
    MOVE G6D, 102,  79, 145, 93, 100, 100
    MOVE G6A,83,  75, 140,  96, 115, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100    
    WAIT

    SPEED 10
    MOVE G6D,98,  79, 145,  93, 100, 100
    MOVE G6A,98,  73, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100    
    WAIT

    SPEED 9
    MOVE G6D,100,  73, 145,  93, 100, 100
    MOVE G6A,100,  79, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100    
    WAIT

    RETURN
    '***********************************************          
GO_FRONT4:

    GOSUB 자이로ON

    GOSUB All_motor_mode3

    SPEED 4
    '오른쪽기울기
    MOVE G6A, 88,  71, 152,  91, 110
    MOVE G6D,106,  76, 146,  93,  96
    MOVE G6B,100,  35,  80, 100
    MOVE G6C,100,  35,  80, 100
    WAIT

    SPEED 10'보행속도
    '왼발들기
    MOVE G6A, 90, 90, 125, 105, 110
    MOVE G6D,108,  79, 146,  93,  90
    MOVE G6B,90
    MOVE G6C,110
    WAIT

    SPEED 12
    '왼발뻣어착지
    MOVE G6A, 88,  47, 155, 113, 110
    MOVE G6D,110,  77, 146,  93,  94
    WAIT

    SPEED 5
    '왼발중심이동
    MOVE G6A,108,  76, 144, 99,  93
    MOVE G6D,85, 93, 155,  70, 112
    WAIT

    SPEED 12
    '오른발들기10
    MOVE G6A,111,  78, 145,  95, 94
    MOVE G6D,90, 105, 105, 112, 114
    MOVE G6B,110
    MOVE G6C,90
    WAIT

    SPEED 12
    '오른발뻣어착지
    MOVE G6D,90,  37, 163, 113, 107
    MOVE G6A,108,  77, 146,  93,  100
    WAIT

    SPEED 5
    '오른발중심이동
    MOVE G6D,100,  72, 144, 99,  97
    MOVE G6A, 95, 93, 155,  70, 100
    WAIT

    SPEED 12
    '왼발들기10
    MOVE G6A, 90, 95, 115, 110, 108
    MOVE G6D,105,  75, 146,  95,  94
    MOVE G6B, 90
    MOVE G6C,110
    WAIT

    SPEED 12
    '왼발뻣어착지
    MOVE G6A, 90,  55, 150, 110, 105
    MOVE G6D,105,  77, 146,  96,  92
    WAIT

    SPEED 5
    '왼발뻣어착지
    MOVE G6A, 96,  70, 148, 97, 98
    MOVE G6D,100,  85, 150,  86,  101
    WAIT

    SPEED 5
    '왼발중심이동
    MOVE G6A,103,  85, 144, 74,  96
    MOVE G6D,95, 93, 155,  68, 108
    WAIT

    SPEED 5
    '오른발들기10
    MOVE G6A,108,  77, 146,  91, 96
    MOVE G6D,95, 90, 120, 112, 108
    MOVE G6B,110
    MOVE G6C,90
    WAIT

    SPEED 5
    MOVE G6A,100,  78, 145,  93, 100, 100
    MOVE G6D,100,  78, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100
    MOVE G6C,100,  35,  80, 100
    WAIT

    RETURN
    '**********************************************
LSWalk: ''new!~

    GOSUB 자이로OFF

    GOSUB Leg_motor_mode2

    SPEED 12
    MOVE G6A, 91,  91, 120, 104, 102, 100	
    MOVE G6D,100,  76, 146,  93, 101, 100	
    WAIT

    SPEED 15
    MOVE G6A,98,  76, 146,  95, 100, 100
    MOVE G6D,98,  76, 146,  93, 100, 100
    WAIT

    SPEED 8
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    WAIT

    RETURN
    '**********************************************
RSWalk: ''new!~

    GOSUB 자이로OFF

    GOSUB Leg_motor_mode2

    SPEED 12
    MOVE G6D, 91,  91, 120, 102, 102, 100	
    MOVE G6A,100,  76, 146,  93, 101, 100	
    WAIT

    SPEED 15
    MOVE G6D,98,  76, 146,  95, 100, 100
    MOVE G6A,98,  76, 146,  93, 100, 100
    WAIT

    SPEED 8
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6A,100,  76, 145,  93, 100, 100
    WAIT

    RETURN
    '**********************************************
LWalk2: 'hyun각도로정면 보고 걷기

    GOSUB 자이로OFF

    GOSUB Leg_motor_mode2

    SPEED 12
    MOVE G6A, 93,  88, 125, 104, 104, 100
    MOVE G6D,103,  76, 145,  96, 104, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT

    SPEED 12
    MOVE G6A, 102,  78, 145, 94, 100, 100
    MOVE G6D,90,  80, 140,  96, 107, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT

    SPEED 15
    MOVE G6A,98,  76, 145,  93, 102, 100
    MOVE G6D,98,  76, 145,  93, 102, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT

    SPEED 8
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT

    RETURN

    '**********************************************
LWalk3: 'kyeong각도로 앞쪽 걷기

    GOSUB 자이로OFF

    GOSUB Leg_motor_mode2

    SPEED 12
    MOVE G6A, 93,  88, 125, 104, 104, 100
    MOVE G6D,103,  76, 145,  96, 104, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT

    SPEED 12
    MOVE G6A, 102,  78, 145, 94, 100, 100
    MOVE G6D,90,  80, 140,  96, 107, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT

    SPEED 15
    MOVE G6A,98,  76, 145,  93, 102, 100
    MOVE G6D,98,  76, 145,  93, 102, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT

    SPEED 8
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT
    RETURN

    '*******************************************
RWalk:
    GOSUB 자이로OFF

    GOSUB Leg_motor_mode2

    SPEED 12
    MOVE G6D, 93,  86, 125, 102, 104, 100
    MOVE G6A,103,  76, 145,  96, 103, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    SPEED 12
    MOVE G6D, 102,  78, 145, 94, 100, 100
    MOVE G6A,90,  80, 140,  96, 107, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    SPEED 15
    MOVE G6D,98,  76, 145,  93, 102, 100
    MOVE G6A,98,  76, 145,  93, 102, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    SPEED 8
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    RETURN
    '**********************************************
RWalk2: 'hyun각도로 숙임
    GOSUB 자이로OFF

    GOSUB Leg_motor_mode2

    SPEED 12
    MOVE G6D, 93,  86, 125, 102, 104, 100
    MOVE G6A,103,  76, 145,  96, 103, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT

    SPEED 12
    MOVE G6D, 102,  78, 145, 94, 100, 100
    MOVE G6A,90,  80, 140,  96, 107, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT

    SPEED 15
    MOVE G6D,98,  76, 145,  93, 102, 100
    MOVE G6A,98,  76, 145,  93, 102, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT

    SPEED 8
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 140, 100
    WAIT

    RETURN
    '***********************************************
RWalk3: 'kyeong각도로 숙이고 걷기

    GOSUB 자이로OFF

    GOSUB Leg_motor_mode2

    SPEED 12
    MOVE G6D, 93,  86, 125, 102, 104, 100
    MOVE G6A,103,  76, 145,  96, 103, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT

    SPEED 12
    MOVE G6D, 102,  78, 145, 94, 100, 100
    MOVE G6A,90,  80, 140,  96, 107, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT

    SPEED 15
    MOVE G6D,98,  76, 145,  93, 102, 100
    MOVE G6A,98,  76, 145,  93, 102, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT

    SPEED 8
    MOVE G6D,100,  76, 145,  93, 100, 100
    MOVE G6A,100,  76, 145,  93, 100, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT

    RETURN
    '***********************************************
milkLturn20:

    GOSUB Leg_motor_mode6

    SPEED 8
    MOVE G6A,95,  96, 145,  73, 105, 100
    MOVE G6D,95,  56, 145,  113, 105, 100
    MOVE G6B,190, 15,  55, 100, 170, 100
    MOVE G6C,190,  15,  55, 100, 170, 100


    WAIT

    SPEED 12
    MOVE G6A,93,  96, 145,  73, 105, 100
    MOVE G6D,93,  56, 145,  113, 105, 100
    MOVE G6B,190, 15,  55, 100, 170, 100
    MOVE G6C,190,  15,  55, 100, 170, 100
    WAIT

    SPEED 6
    MOVE G6A,101,  76, 146,  93, 98, 100
    MOVE G6D,101,  76, 146,  93, 98, 100
    MOVE G6B,190, 15,  55, 100, 170, 100
    MOVE G6C,190,  15,  55, 100, 170, 100

    WAIT

    RETURN
    '**********************************************
line_왼쪽턴20:


    GOSUB Leg_motor_mode6

    SPEED 8
    MOVE G6A,95,  96, 145,  73, 105, 100
    MOVE G6D,95,  56, 145,  113, 97, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 170, 100
    WAIT

    SPEED 12
    MOVE G6A,93,  96, 145,  73, 105, 100
    MOVE G6D,93,  56, 145,  113, 97, 100
    MOVE G6B, ,  ,  , 100, 100, 100
    MOVE G6C,,  ,  , 100, 170, 100
    WAIT

    SPEED 6
    MOVE G6A,101,  76, 146,  93, 98, 100
    MOVE G6D,101,  76, 146,  93, 98, 100
    MOVE G6B,,  ,  , 100, 100, 100
    MOVE G6C,,  ,  , 100, 170, 100

    WAIT

    RETURN
    '***************************************
Lturn20:


    GOSUB Leg_motor_mode6

    SPEED 8
    MOVE G6A,95,  96, 145,  73, 105, 100
    MOVE G6D,95,  56, 145,  113, 105, 100
    MOVE G6B,100,  35,  80, 100, 100, 100
    MOVE G6C,100,  35,  80, 100, 110, 100
    WAIT

    SPEED 12
    MOVE G6A,93,  96, 145,  73, 105, 100
    MOVE G6D,93,  56, 145,  113, 105, 100
    MOVE G6B, ,  ,  , 100, 100, 100
    MOVE G6C,,  ,  , 100, 110, 100
    WAIT

    SPEED 6
    MOVE G6A,101,  76, 146,  93, 98, 100
    MOVE G6D,101,  76, 146,  93, 98, 100
    MOVE G6B,,  ,  , 100, 100, 100
    MOVE G6C,,  ,  , 100, 110, 100

    WAIT



    RETURN
    '***************************************
milkRturn20:

    GOSUB Leg_motor_mode6

    SPEED 8
    MOVE G6A,95,  56, 145,  113, 105, 100
    MOVE G6D,95,  96, 145,  73, 105, 100
    MOVE G6B,190, 15,  55, 100, 170, 100
    MOVE G6C,190,  15,  55, 100, 170, 100
    WAIT

    SPEED 12
    MOVE G6A,93,  56, 145,  113, 105, 100
    MOVE G6D,93,  96, 145,  73, 105, 100
    MOVE G6B,190, 15,  55, 100, 170, 100
    MOVE G6C,190,  15,  55, 100, 170, 100
    WAIT

    SPEED 6
    MOVE G6A,101,  76, 146,  93, 98, 100
    MOVE G6D,101,  76, 146,  93, 98, 100
    MOVE G6B,190, 15,  55, 100, 170, 100
    MOVE G6C,190,  15,  55, 100, 170, 100

    WAIT

    RETURN
    '********************************************
line_오른쪽턴20:

    GOSUB Leg_motor_mode6

    SPEED 8
    MOVE G6A,95,  56, 145,  113, 97, 100
    MOVE G6D,95,  96, 145,  73, 105, 100
    MOVE G6B,100
    MOVE G6C,100, , , ,170
    WAIT

    SPEED 12
    MOVE G6A,93,  56, 145,  113, 97, 100
    MOVE G6D,93,  96, 145,  73, 105, 100
    MOVE G6C, , , , , 170
    WAIT

    SPEED 6
    MOVE G6A,101,  76, 146,  93, 98, 100
    MOVE G6D,101,  76, 146,  93, 98, 100
    MOVE G6C, , , , , 170
    WAIT

    RETURN
    '********************************************
Rturn20:

    GOSUB Leg_motor_mode6

    SPEED 8
    MOVE G6A,95,  56, 145,  113, 105, 100
    MOVE G6D,95,  96, 145,  73, 105, 100
    MOVE G6B,100
    MOVE G6C,100, , , ,110
    WAIT

    SPEED 12
    MOVE G6A,93,  56, 145,  113, 105, 100
    MOVE G6D,93,  96, 145,  73, 105, 100
    MOVE G6C, , , , , 110
    WAIT

    SPEED 6
    MOVE G6A,101,  76, 146,  93, 98, 100
    MOVE G6D,101,  76, 146,  93, 98, 100
    MOVE G6C, , , , , 110
    WAIT

    RETURN
    '********************************************


MAIN: '라벨설정

    '**** 입력된 A값이 0 이면 MAIN 라벨로 가고
    '**** 1이면 KEY1 라벨, 2이면 key2로... 가는문

    'GOSUB 전압체크
    ' GOSUB 앞뒤기울기측정
    '  GOSUB 좌우기울기측정

    IF 모터ONOFF = 1  THEN
        DELAY 200
        OUT 52,0
        DELAY 200
    ENDIF

    ERX 4800,A,MAIN	'
    'A = REMOCON(1)

    A_old = A

MAIN2:

    ON A GOTO MAIN,KEY1,KEY2,KEY3,KEY4,KEY5,KEY6,KEY7,KEY8,KEY9,KEY10,KEY11,KEY12,KEY13,KEY14,KEY15,KEY16,KEY17,KEY18,KEY19,KEY20,KEY21,KEY22,KEY23,KEY24,KEY25,KEY26,KEY27, KEY28, KEY29, KEY30, KEY31, KEY32, KEY33, KEY34, KEY35, KEY36, KEY37, KEY38, KEY39, KEY40, KEY41, KEY42, KEY43, KEY44, KEY45, KEY46, KEY47, KEY48, KEY49, KEY50, KEY51, KEY52, KEY53, KEY54, KEY55, KEY56, KEY57, KEY58, KEY59, KEY60, KEY61, KEY62,KEY63,KEY64,KEY65,KEY66,KEY67, KEY68, KEY69, KEY70, KEY71

    GOTO MAIN	
    '*******************************************
    '		MAIN 라벨로 가기
    '*******************************************

KEY1:
    ETX  4800,1
    GOSUB EAST
    GOTO RX_EXIT

KEY2:
    ETX  4800,2
    GOSUB WEST
    GOTO RX_EXIT

KEY3:
    ETX  4800,3
    GOSUB SOUTH
    GOTO RX_EXIT

KEY4:
    ETX  4800,4
    GOSUB NORTH
    GOTO RX_EXIT

KEY5:
    ETX  4800,5
    GOSUB 기본자세
    GOTO RX_EXIT

KEY6:
    ETX  4800,6
    GOSUB CDWalk
    GOTO RX_EXIT

KEY7:
    ETX  4800,7
    GOSUB jiwon
    GOTO RX_EXIT

KEY8:
    ETX  4800,8
    GOSUB milkwalkfront
    GOTO RX_EXIT

KEY9:
    ETX  4800,9
    GOSUB jiwondown
    GOTO RX_EXIT

KEY10: '0
    ETX  4800,10
    GOSUB dooropen
    GOTO RX_EXIT

KEY11: ' ▲
    ETX  4800,11
    GOSUB WooWalk2
    GOTO RX_EXIT

KEY12: ' ▼
    ETX  4800,12
    GOSUB backstepleft
    GOTO RX_EXIT
KEY13: '▶
    ETX  4800,13
    GOSUB Rwalk
    GOTO RX_EXIT

KEY14: ' ◀
    ETX  4800,14
    GOSUB Lwalk
    GOTO RX_EXIT

KEY15: ' A
    ETX  4800,15
    GOSUB hyunleft
    GOTO RX_EXIT

KEY16: ' POWER
    ETX  4800,16  '원래 Swalk자리 
    GOTO RX_EXIT

KEY17: ' C
    ETX  4800,17
    GOSUB kyeongleft
    GOTO RX_EXIT

KEY18: ' E
    ETX  4800,18	
    GOSUB hyunfront
    GOTO RX_EXIT

KEY19: ' P2
    ETX  4800,19
    GOSUB safe
    GOTO RX_EXIT

KEY20: ' B	
    ETX  4800,20
    GOSUB hyunright
    GOTO RX_EXIT

KEY21: ' △
    ETX  4800,21
    GOSUB Turnleft60
    GOTO RX_EXIT

KEY22: ' *
    ETX  4800,22
    GOSUB milkSWalk
    GOTO RX_EXIT

KEY23: ' G
    ETX  4800,23
    GOSUB kyeongfront
    GOTO RX_EXIT

KEY24: ' #
    ETX  4800,24
    GOSUB armheaddown
    GOTO RX_EXIT

KEY25: ' P1
    ETX  4800,25
    GOSUB danger
    GOTO RX_EXIT

KEY26: ' ■
    ETX  4800,26
    GOSUB GO_FRONT2
    GOTO RX_EXIT

KEY27: ' D
    ETX  4800,27
    GOSUB kyeongright
    GOTO RX_EXIT

KEY28: ' ◁
    ETX  4800,28
    GOSUB line_왼쪽턴20
    GOTO RX_EXIT

KEY29: ' □
    ETX  4800,29
    GOSUB Lturn90
    GOTO RX_EXIT

KEY30: ' ▷
    ETX  4800,30
    GOSUB line_오른쪽턴20
    GOTO RX_EXIT

KEY31: ' ▽
    ETX  4800,31
    GOSUB Turnright60
    GOTO RX_EXIT

KEY32: ' F
    ETX  4800,32
    GOSUB Go_front2
    GOTO RX_EXIT

KEY33: '
    ETX  4800,33
    GOSUB Swalk2
    GOTO RX_EXIT

KEY34: '
    ETX  4800,34
    GOSUB Swalk3
    GOTO RX_EXIT

KEY35: '
    ETX  4800,35
    GOSUB Lwalk2
    GOTO RX_EXIT

KEY36: '
    ETX  4800,36
    GOSUB Lwalk3
    GOTO RX_EXIT

KEY37: '
    ETX  4800,37
    GOSUB Rwalk2
    GOTO RX_EXIT

KEY38: '
    ETX  4800,38
    GOSUB Rwalk3
    GOTO RX_EXIT

KEY39: '
    ETX  4800,39
    GOSUB Go_front22
    GOTO RX_EXIT

KEY40: '
    ETX  4800,40
    GOSUB Go_front23
    GOTO RX_EXIT

KEY41: '
    ETX  4800,41
    GOSUB killeft
    GOTO RX_EXIT

KEY42: '
    ETX  4800,42
    GOSUB kilright
    GOTO RX_EXIT

KEY43: '
    ETX  4800,43
    GOSUB kilfront
    GOTO RX_EXIT

KEY44: '
    ETX  4800,44
    GOSUB Lastdooropen
    GOTO RX_EXIT

KEY45: '
    ETX  4800,45
    GOSUB backstepleft2
    GOTO RX_EXIT

KEY46:
    ETX  4800,46
    GOSUB backstepleft3
    GOTO RX_EXIT

KEY47:
    ETX  4800,47
    GOSUB milkLturn20
    GOTO RX_EXIT

KEY48:
    ETX  4800,48
    GOSUB milkRturn20
    GOTO RX_EXIT

KEY49:
    ETX  4800,49
    GOSUB MTurnLeft60
    GOTO RX_EXIT

KEY50:
    ETX  4800,50
    GOSUB MTurnRight60
    GOTO RX_EXIT

KEY51:
    ETX  4800,51
    GOSUB Lturn20
    GOTO RX_EXIT

KEY52:
    ETX  4800,52
    GOSUB Rturn20
    GOTO RX_EXIT

KEY53:
    ETX  4800,53
    GOSUB TurnLeft603
    GOTO RX_EXIT

KEY54:
    ETX  4800,54
    GOSUB TurnRight603
    GOTO RX_EXIT

KEY55:
    ETX  4800,55
    GOSUB 기본125
    GOTO RX_EXIT

KEY56:
    ETX  4800,56
    GOSUB 기본130
    GOTO RX_EXIT

KEY57:
    ETX  4800,57
    GOSUB ST
    GOTO RX_EXIT

KEY58:
    ETX  4800,58
    GOSUB Aland
    GOTO RX_EXIT

KEY59:
    ETX  4800,59
    GOSUB Bland
    GOTO RX_EXIT

KEY60:
    ETX  4800,60
    GOSUB Cland
    GOTO RX_EXIT

KEY61:
    ETX  4800,61
    GOSUB Dland
    GOTO RX_EXIT

KEY62:
    ETX  4800,62
    GOSUB kilfront2
    GOTO RX_EXIT

KEY63:
    ETX 4800,63
    GOSUB 왼쪽턴180_LOOP_3
    GOTO RX_EXIT

KEY64:
    ETX 4800,64
    GOSUB 왼쪽턴180_LOOP_4
    GOTO RX_EXIT

KEY65:
    ETX 4800,65
    GOSUB 오른쪽턴100_LOOP
    GOTO RX_EXIT

KEY66:
    ETX 4800,66
    GOSUB LWALK5
    GOTO RX_EXIT

KEY67:
    ETX 4800,67
    GOSUB ML180_LOOP_4
    GOTO RX_EXIT
 
KEY68:
    ETX 4800,68
    GOSUB Rwalk5
    GOTO RX_EXIT
    
KEY69:
    ETX 4800,69
    GOSUB Swalk
    GOTO RX_EXIT
     
KEY70:
    ETX 4800,70
    GOSUB KLwalk5
    GOTO RX_EXIT  

KEY71:
    ETX 4800,71
    GOSUB KRwalk5
    GOTO RX_EXIT  
           
    END